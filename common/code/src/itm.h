...
#define ITM_ENABLE_STIMULUS_PORTS_ALL 0xFFFFFFFF
    ...
    /**
     * @brief Writes 32-bit float raw bytes to stimulus port
     *
     * @param port Port
     * @param value Value to be written
     */
    static inline void ITMWriteF32(uint8_t port, f32 value);
    ...
    void ITMWriteF32(uint8_t port, f32 value)
    {
        if(!ITMIsPortEnabled(port))
        {
            return;
